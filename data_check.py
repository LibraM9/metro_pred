...
train_in["date"] = train_in["time"].apply(lambda x: x.split(" ")[0])
train_in["time"] = train_in["time"].apply(lambda x: x.split(" ")[1])
train_out["date"] = train_out["time"].apply(lambda x: x.split(" ")[0])
...
def station_plot(station):
    ...
    for i in range(len(time_list_sub)):
        ...
        temp_in = train_in[(train_in["stationID"] == station) & (train_in["time"] == time)]
        temp_in = temp_in.reset_index()
        temp_out = train_out[(train_out["stationID"] == station) & (train_out["time"] == time)]
